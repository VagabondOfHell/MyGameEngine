...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow )
{
    ...
    g.hwnd = CreateWindow(TEXT("GameEngine"),
                          TEXT("GameEngine"),
                          WS_OVERLAPPEDWINDOW,
	...
	OpenGLRenderer glRenderer = OpenGLRenderer();

	//Set up the renderer
	glRenderer.SetWindowSize(g.width, g.height);
	glRenderer.SetContextHandle(g.hdc);
	glRenderer.SetViewport(0, 0, g.width, g.height);
	
	app.SetRenderer(&glRenderer);
	
	app.OnStart();

	float lastMilliseconds = 0;
	float currentMilliseconds = 0;
	float millisecondsThisFrame = 0;
    ...
    while( 1 )
    {
        ...
        else
        {
			...
			currentMilliseconds = app.GetElapsedMilliseconds();

			millisecondsThisFrame = currentMilliseconds - lastMilliseconds;

			Logger::GetInstance().LogMessage("Milliseconds Difference: " + std::to_string(millisecondsThisFrame), LogPriority::Error);

			app.PreRender(millisecondsThisFrame);
			app.Render();
			app.PostRender();
...
LRESULT CALLBACK WndProc(   HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam ) 
{
	//Handle the windows procedure message
    switch( message )
    {
    case WM_CREATE:
        Beep( 50, 10 );
